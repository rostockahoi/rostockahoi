...
(function() {
    ...
    window.Typewriter = function Typewriter(element, options) {
        this._settings = { cursorAnimationPaused: false, opacityIncreasing: false, currentOpacity: 1, delayedQue: [], delayItemsCount: 0, eventQue: [], calledEvents: [], eventRunning: false, timeout: false, delayExecution: false, fps: (60 / 1000), typingFrameCount: 0, stringToTypeHTMLArray: [], currentTypedCharacters: [], typing: false, usedIDs: [], charAmountToDelete: false, userOptions: {}, eventLoopRerun: 0 };
        if (!element) {
        ...
        this.default_options = { strings: false, cursorClassName: 'typewriter-cursor', cursor: '|', animateCursor: true, blinkSpeed: 50, typingSpeed: 'natural', deleteSpeed: 'natural', charSpanClassName: 'typewriter-char', wrapperClassName: 'typewriter-wrapper', loop: false, autoStart: false, devMode: false };
        this.options = this._setupOptions(options);
        this.el = element;
        this._setupTypwriterWrapper();
        this._startCursorAnimation();
        if (this.options.autoStart === true && this.options.strings) { this.typeOutAllStrings(); }
    };
    ...
    TypewriterPrototype.start = function() { this._startEventLoop();
        return this; };
    TypewriterPrototype.rerun = function() { this._addToEventQue(this._rerunCalledEvents);
        return this; };
    ...
    TypewriterPrototype.deleteAll = function() { this._addToEventQue([this._deleteChars, ['all']]);
        return this; };
    TypewriterPrototype.deleteChars = function(amount) { this._addToEventQue([this._deleteChars, [amount]]);
        return this; };
    TypewriterPrototype.pauseFor = function(ms) { this._addToEventQue([this._pauseFor, [ms]]);
        return this; };
    TypewriterPrototype.typeOutAllStrings = function() {
        var characters_array = this._getStringsAsCharsArray();
        if (characters_array.length === 1) { this._typeCharacters(characters_array[0]); } else {
            for (var i = 0, length = characters_array.length; i < length; i++) { this._addToEventQue([this._typeCharacters, [characters_array[i]]]);
    ...
    TypewriterPrototype.changeBlinkSpeed = function(new_speed) {
        ...
        this.changeSettings({ blinkSpeed: new_speed });
        return this;
    };
    TypewriterPrototype.changeTypingSpeed = function(new_speed) {
        ...
        var new_settings = { typingSpeed: new_speed };
        this.changeSettings({ typingSpeed: new_speed });
        return this;
    };
    TypewriterPrototype.changeDeleteSpeed = function(new_speed) {
        ...
        this.changeSettings({ changeDeleteSpeed: new_speed });
        return this;
    };
    TypewriterPrototype._rerunCalledEvents = function() {
        if (this._settings.currentTypedCharacters.length > 0) { this.deleteAll();
            this._resetEventLoop('rerunCalledEvents'); } else { this._settings.eventQue = this._settings.calledEvents;
    ...
    TypewriterPrototype._deleteChars = function(amount) {
        if (amount) { this._settings.charAmountToDelete = amount; }
    ...
    TypewriterPrototype._pauseFor = function(ms) {
        ...
        setTimeout(function() { self._resetEventLoop('pauseFor'); }, ms); };
    TypewriterPrototype._changeSettings = function(new_settings) { this.options = this._setupOptions(JSON.parse(new_settings[0]));
        this._resetEventLoop('changeSettings');
        if (this.options.devMode) { console.log('New settings', this.options); } };
    TypewriterPrototype._deletingCharAnimationFrame = function() {
        ...
        if (!self._settings.charAmountToDelete || self._settings.charAmountToDelete === 0 || current_typed_char_ids === 0) { self._resetEventLoop('deletingCharAnimationFrame');
            return true; }
        if (delete_speed == 'natural') { delete_speed = self._randomInteger(50, 150); }
        if (char_amount_to_delete == 'all') { char_amount_to_delete = current_typed_char_ids.length;
        ...
        setTimeout(function() {
            if (self._settings.charAmountToDelete) {
                ...
                if (char_to_delete_el) {
                    ...
                    if (self.options.devMode) { console.log('Deleted char with ID', get_last_typed_char); } } }
    ...
    TypewriterPrototype._setupOptions = function(new_options) {
        var merged_options = {};
        for (var attrname in this.default_options) { merged_options[attrname] = this.default_options[attrname]; }
        if (this._settings.userOptions) {
            for (var attrname in this._settings.userOptions) { merged_options[attrname] = this._settings.userOptions[attrname]; } }
        for (var attrname in new_options) { merged_options[attrname] = new_options[attrname]; }
        return merged_options;
    }
    TypewriterPrototype._addToEventQue = function(event) { this._settings.eventQue.push(event);
        if (this._settings.eventQue.length > 0 && !this._settings.eventRunning && this.options.autoStart) { this._startEventLoop(); } };
    TypewriterPrototype._startEventLoop = function() {
        if (this.options.devMode) { console.log('Event loop started.'); }
        if (!this._settings.eventRunning) {
            if (this._settings.eventQue.length > 0) { this.eventLoopRerun = 0;
                var first_event = this._settings.eventQue[0];
                if (typeof first_event == 'function') { this._settings.eventRunning = true;
                    this._settings.calledEvents.push(first_event);
                    this._settings.eventQue.splice(0, 1);
                    first_event.call(this);
                    if (this.options.devMode) { console.log('Event started.'); } } else if (first_event instanceof Array) {
                    if (typeof first_event[0] == 'function' && first_event[1] instanceof Array) { this._settings.eventRunning = true;
                        this._settings.calledEvents.push(first_event);
                        this._settings.eventQue.splice(0, 1);
                        first_event[0].call(this, first_event[1]);
                        if (this.options.devMode) { console.log('Event started.'); } } } }
        ...
        if (!this._settings.eventRunning && this._settings.eventQue.length <= 0) {
            ...
            setTimeout(function() {
                if (self.options.loop) {
                    self.eventLoopRerun++;
                    if (self.options.devMode) { console.log('Before Loop State', self._settings); }
                    if (self.eventLoopRerun > 4) { console.error('Maximum amount of loop retries reached.');
                        self._stopEventLoop(); } else {
                        if (self.options.devMode) { console.log('Looping events.'); }
    ...
    TypewriterPrototype._resetEventLoop = function(name) {
        ...
        this._startEventLoop();
        if (this.options.devMode) { console.log(event_name, 'Finished'); } };
    TypewriterPrototype._stopEventLoop = function() { window.cancelAnimationFrame(this._eventQueAnimation);
        if (this.options.devMode) { console.log('Event loop stopped.'); } };
    ...
    TypewriterPrototype._typeCharacters = function(characters_array) { this._settings.stringToTypeHTMLArray = this._convertCharsToHTML(characters_array);
    ...
    TypewriterPrototype._typingAnimationFrame = function(total_items) {
        ...
        var typewriter_wrapper_class_name = self.options.wrapperClassName;
        if (self._settings.stringToTypeHTMLArray.length == 0) { window.cancelAnimationFrame(self._typingAnimation);
        ...
        setTimeout(function() {
            ...
            if (self.options.devMode) { console.log('Typed', item_to_type); } }, typing_speed);
    };
    TypewriterPrototype._convertCharsToHTML = function(chars) {
        ...
        for (var i = 0, length = chars_array.length; i < length; i++) {
            ...
            chars_html_wrap_array.push({ id: char_id, el: char_element }); }
    ...
    TypewriterPrototype._getStringsAsCharsArray = function() {
        ...
        for (var i = 0, length = this.options.strings.length; i < length; i++) {
            ...
            if (!string_chars) { console.error('Please enter only strings.');
                break; }
    ...
    TypewriterPrototype._cursorAnimationFrame = function() {
        if (!this._settings.cursorAnimationPaused) {
            ...
            if (this._settings.opacityIncreasing == true) {
                if (this._settings.currentOpacity >= 1) { this._settings.opacityIncreasing = false;
                    this._settings.currentOpacity = 1; }
                this._settings.currentOpacity += opacity_amount;
            }
            if (this._settings.opacityIncreasing == false) {
                if (this._settings.currentOpacity <= 0) { this._settings.opacityIncreasing = true;
    ...
    TypewriterPrototype._startCursorAnimation = function() {
        ...
        if (this.options.animateCursor) { this._cursorAnimation = window.requestAnimationFrame(this._cursorAnimationFrame.bind(this)); } };
    TypewriterPrototype._pauseCursorAnimation = function() {
        if (!this._settings.cursorAnimationPaused) { window.cancelAnimationFrame(this._cursorAnimation);
    ...
    TypewriterPrototype._randomID = function() {
        ...
        for (var i = 0; i < this._randomInteger(5, 15); i++) { text += possible.charAt(Math.floor(Math.random() * possible.length)); }
        return text;
    };
    TypewriterPrototype._generateUniqueID = function() {
        var temp_id = this._randomID();
        if (this._settings.usedIDs.indexOf(temp_id) == -1) { this._settings.usedIDs.push(temp_id);
            return temp_id; }
